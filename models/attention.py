...
class LocationSensitiveAttention(BahdanauAttention):
  ...
  577–585.
	...
	def __init__(self,
		...
		normalization_function = self._smoothing_normalization if (smoothing == True) else None
	...
	def __call__(self, query, state):
		...
		with variable_scope.variable_scope(None, "Location_Sensitive_Attention", [query]):
			...
			energy = self._location_sensitive_score(processed_query, processed_location_features)
	...
	def _location_sensitive_score(self, W_query, W_fil):
		"""Impleements Bahdanau-style (cumulative) scoring function.
        This attention is described in:
            J. K. Chorowski, D. Bahdanau, D. Serdyuk, K. Cho, and Y. Ben-
        gio, “Attention-based models for speech recognition,” in Ad-
        vances in Neural Information Processing Systems, 2015, pp.
        ...
        #############################################################################
                hybrid attention (content-based + location-based)
                                f = F * α_{i-1}
        energy = dot(v_a, tanh(self.keys(h_enc) + W_query(h_dec) + W_fil(f) + b_a))
        #############################################################################
        Args:
		...
		dtype = W_query.dtype
		# self.keys of type: Tensor, shape '[batch_size, max_time, attention_dim]', typically the encoder outputs.
		num_units = self.keys.shape[-1].value or array_ops.shape(self.keys)[-1]
		...
		return tf.reduce_sum(v_a * tf.tanh(self.keys + W_query + W_fil + b_a), [2])
