...
class TacotronMelPML():
  def __init__(self, hparams):
    self._hparams = hparams


  def initialize(self, inputs, input_lengths, mel_targets=None, pml_targets=None):
    '''Initializes the model for inference.

    Sets "mel_outputs", "pml_outputs", and "alignments" fields.
    ...
    with tf.variable_scope('inference') as scope:
      ...
      post_outputs = post_cbhg(mel_outputs, hp.num_mels, is_training,            # [N, T_out, postnet_depth=256]
                               hp.postnet_depth)
      pml_outputs = tf.layers.dense(post_outputs, hp.pml_dimension)                # [N, T_out, F]
  ...
  def add_loss(self):
    ...
    with tf.variable_scope('loss') as scope:
      ...
      self.loss = self.mel_loss + self.pml_loss
