...
def synthesize(fs, f0s, SPEC, NM=None, wavlen=None
    ...
    dampinhwin = dampinhwin[:int((len(dampinhwin) - 1) / 2) + 1]

    for n, t in enumerate(f0s[:,0]):
        f0 = f0s[n,1]

        if verbose>1:
            print("\rPM Synthesis (python) t={:4.3f}s f0={:3.3f}Hz               ".format(t,f0))
        ...
        S *= np.exp((delay*2j*np.pi/dftlen)*np.arange(int(dftlen / 2) + 1))
        ...
        GN *= np.exp((delay*2j*np.pi/dftlen)*np.arange(int(dftlen / 2) + 1))
