...
void fvoper(FVECTOR x, char *op, FVECTOR y)
{
	...
	if (reverse) {
		...
		if (k < y->length) {
			...
			if (x->data[k] == 0.0 && x->imag[k] == 0.0) {
			    if (sp_warning)
				printmsg(stderr, "warning: fvoper: divide by zero\n");
	...
	printmsg(stderr, "fvoper: unknouwn operation: %s\n", op2);
	exit(1);
    }
...
void dvoper(DVECTOR x, char *op, DVECTOR y)
{
	...
	if (reverse) {
		...
		if (k < y->length) {
			...
			if (x->data[k] == 0.0 && x->imag[k] == 0.0) {
			    if (sp_warning)
				printmsg(stderr, "warning: dvoper: divide by zero\n");
	...
	printmsg(stderr, "dvoper: unknouwn operation: %s\n", op2);
	exit(1);
    }
...
void fvscoper(FVECTOR x, char *op, float t)
{
	...
	for (k = 0; k < x->length; k++) {
		...
		if (x->imag != NULL) {
		    if (x->data[k] == 0.0 && x->imag[k] == 0.0) {
			if (sp_warning)
			    printmsg(stderr, "warning: fvscoper: divide by zero\n");
	...
	printmsg(stderr, "fvscoper: unknouwn operation: %s\n", op2);
	exit(1);
    }
...
void dvscoper(DVECTOR x, char *op, double t)
{
	...
	for (k = 0; k < x->length; k++) {
		...
		if (x->imag != NULL) {
		    if (x->data[k] == 0.0 && x->imag[k] == 0.0) {
			if (sp_warning)
			    printmsg(stderr, "warning: dvscoper: divide by zero\n");
	...
	printmsg(stderr, "dvscoper: unknouwn operation: %s\n", op2);
	exit(1);
    }
...
void svoper(SVECTOR a, char *op, SVECTOR b)
{
	...
	if (reverse) {
		...
		if (k < b->length) {
			...
			if (sp_warning)
			    printmsg(stderr, "warning: svoper: divide by zero\n");
	...
	printmsg(stderr, "svoper: unknouwn operation: %s\n", op2);
	exit(1);
    }
...
void lvoper(LVECTOR a, char *op, LVECTOR b)
{
	...
	if (reverse) {
		...
		if (k < b->length) {
			...
			if (sp_warning)
			    printmsg(stderr, "warning: lvoper: divide by zero\n");
	...
	printmsg(stderr, "lvoper: unknouwn operation: %s\n", op2);
	exit(1);
    }
...
void svscoper(SVECTOR a, char *op, double t)
{
	...
	for (k = 0; k < a->length; k++) {
		...
		if (a->data[k] != 0.0) {
		    ...
		    if (sp_warning)
			printmsg(stderr, "warning: svscoper: divide by zero\n");
	...
	printmsg(stderr, "svscoper: unknouwn operation: %s\n", op2);
	exit(1);
    }
...
void lvscoper(LVECTOR a, char *op, double t)
{
	...
	for (k = 0; k < a->length; k++) {
		...
		if (a->data[k] != 0.0) {
		    ...
		    if (sp_warning)
			printmsg(stderr, "warning: lvscoper: divide by zero\n");
	...
	printmsg(stderr, "lvscoper: unknouwn operation: %s\n", op2);
	exit(1);
    }
...
DVECTOR xdvsceval(DVECTOR x, char *op, double t)
{
	...
	printmsg(stderr, "xdvsceval: unknouwn operation: %s\n", op);
	exit(1);
    }
...
void fvdecibel(FVECTOR x)
{
	...
	if (x->data[k] <= 0.0) {
	    if (sp_warning)
		printmsg(stderr, "warning: fvdecibel: log of zero\n");
...
void dvdecibel(DVECTOR x)
{
	...
	if (x->data[k] <= 0.0) {
	    if (sp_warning)
		printmsg(stderr, "warning: dvdecibel: log of zero\n");
...
float fvdot(FVECTOR x, FVECTOR y)
{
	...
	printmsg(stderr, "fvdot: vector length must agree\n");
	exit(1);
    } else {
	for (k = 0; k < x->length; k++) {
	    a += x->data[k] * y->data[k];
...
double dvdot(DVECTOR x, DVECTOR y)
{
	...
	printmsg(stderr, "dvdot: vector length must agree\n");
	exit(1);
    } else {
	for (k = 0; k < x->length; k++) {
	    a += x->data[k] * y->data[k];
