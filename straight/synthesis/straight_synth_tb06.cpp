                            ...
                            XBOOL df_flag)      /* using proportional group delay */
{
  ...
  DVECTOR sy = NODATA;
  DVECTOR sy2 = NODATA;
  DVECTOR wlcut = NODATA;
  DVECTOR wlcutfric = NODATA;
  DVECTOR fgw = NODATA;
  DVECTOR han = NODATA;
  DVECTOR rho = NODATA;
  DVECTOR rho2 = NODATA;
  ...
  if (fs == 0.0 || shiftm == 0.0 || pconv == 0.0 || fconv == 0.0 || sconv == 0.0) {
    fprintf(stderr, "straight synth: wrong parameter value\n");
    return NODATA;
  }

  /* memory allocation */
  nsyn = (long)round(sconv * (double)njj * shiftm * fs / 1000.0 + 3.0 * (double)fftl + 1.0);
  sy = xdvzeros(nsyn);
  ...
  /* memory allocation */
  mxv = xdvalloc(f0var->length);
  mxn = xdvalloc(f0var->length);
  ...
  while ((long)idx < maxidx && (long)ceil(iin) < f0l->length - 1) {
    ...
    ii = MIN(MIN(MAX(0.0, iix), (double)njj - 1.0), (double)f0l->length - 1.0);
    lii = (long)round(ii);
    
    // error check
    if (end_flag == XTRUE) 
      break;
    if (lii == n2sgram->row - 1) 
      end_flag = XTRUE;
    ...
    for (k = 0; k < hfftl; k++) 
      wlcut->data[k] = 1.0 / (1.0 + exp(-10.0 *((double)k / (double)fftl * fs - f0 * 0.7) / f0));

    /* calculate cepstrum */
    ccp = ss_xextractcep_tb06(n2sgram, lii, idcv, fftl, wlcut, dmx / 1000000.0);

    if (zp_flag != XTRUE) 
      ss_ceptompc(ccp, fftl);
    ...
    if (rp_flag == XTRUE) {
        if (df_flag == XTRUE) 
          delsp = delfrac * 1000.0 / f0;
        for (k = 0; k < rho->length; k++) 
          rho2->data[k] = mxn->data[lii] + (1 - mxn->data[lii]) * rho->data[k];
    ...
    idx += nf0;
    iin = MIN((double)f0l->length - 1.0, (idx + 1.0) * 1000.0 / fs / shiftm / sconv);
  ...
  for (k = 0; k < hfftl; k++) 
    wlcutfric->data[k] = 1.0 / (1.0 + exp(-14.0 * ((double)k / (double)fftl * fs - lowcutf) / lowcutf));
  ...
  while ((long)idx < maxidx && (long)ii < f0l->length - 1) {
    ii = round(MIN(MIN((double)f0l->length - 1.0, njj - 1.0), (idx + 1.0) * 1000.0/ fs / shiftm / sconv));
    lii = (long)round(ii);
    nidx = (long)round(idx);
    ...
    if (mxn->data[lii] > 0.03) {
      /* calculate cepstrum */
      ccp = ss_xextractcep_tb06(n2sgram, lii, idcv, fftl, wlcutfric, dmx / 100000.0);
    ...
    idx += nf0;
    ii = MIN((double)f0l->length - 1.0, (idx + 1.0) * 1000.0 / fs / shiftm / sconv);
  }
  ...
  sy2 = xdvcut(sy, fftl2, (long)((double)njj * shiftm * fs / 1000.0 * sconv));
...
DVECTOR straight_synth_tb06ca(DMATRIX n2sgram,   /* amplitude spectrogram */
                              ...
                              XBOOL bap_flag,    /* using band-limited aperiodicity */
                              XBOOL fr_flag,     /* using fractional pitch */
                              XBOOL zp_flag,     /* using zero phase */
                              XBOOL rp_flag,     /* using random phase */
                              XBOOL df_flag)     /* using proportional group delay */
{
  ...
  DVECTOR tnx = NODATA;    
  DVECTOR rx = NODATA;
  DVECTOR tmpimap = NODATA;    
  DVECTOR cpimap = NODATA;
  DVECTOR imapx = NODATA;       
  DVECTOR imapy = NODATA;
  DVECTOR imapi = NODATA;    
  DVECTOR rmap = NODATA;
  DVECTOR wnz = NODATA;    
  DVECTOR wpr = NODATA;
  DVECTOR wfv = NODATA;
  ...
  if (ix == -1) 
    ix = k;
  for (; k < cpimap->length; k++) 
    cpimap->data[k] = (double)(f0len - 1);
  imapx = xdvalloc(ix + 1);
  imapy = xdvalloc(ix + 1);
  ...
  for (k = 0; k < imapi->length; k++) 
    imapi->data[k] = (double)k;
  ...
  while ((long)idx < maxidx && (long)ceil(iin) < f0len - 1) {
    ...
    ii = MIN(MIN(MAX(0.0, iix), (double)njj - 1.0), (double)f0len - 1.0);
    lii = (long)round(ii);
    ...
    if (f0l->data[lii] > 0.0) {
      ...
      if(bap_flag == XTRUE){
        ...
        */
        ...
        for (b = 0; b < ap->col; b++) {            
          if((int) numbands != (int) ap->col){          
            // fixed conventional frequency bands              
            if (b == 0) {
              bs = 0; be = fftl / 16;
            } else if (b == 1) {
              bs = fftl / 16;    be = fftl / 8;
            } else if (b == 2) {
              bs = fftl / 8;    be = fftl / 4;
            } else if (b == 3) {
              bs = fftl / 4;     be = fftl * 3 / 8;
            } else {
              bs = fftl * 3 / 8;     be = fftl / 2 + 1;
            }
          ...
          if (sigp <= 0.0)
            wnzv = MIN(1.0, pow(10.0, (ap->data[lii][b]) / 20.0));
          else {
            wnzv = sigmoid(pow(10.0, (ap->data[lii][b]) / 20.0), sigp, 0.25);
            wnzv = MIN(1.0, wnzv);
          }
          wprv = sqrt(MAX(0.0, 1.0 - SQUARE(wnzv)));
          for (k = bs; k < be; k++) {
            wnz->data[k] = wnzv;
            wpr->data[k] = wprv;
            if (k != 0 && k != fftl / 2) {
              wnz->data[fftl - k] = wnzv;
              wpr->data[fftl - k] = wprv;
            }
          }
        }
        
      }else{
        // aperiodicity for each fft bin 
        wnz->data[0] = pow(10.0, ap->data[lii][idcv->data[0]] / 20.0);
        wpr->data[0] = sqrt(MAX(0.0, 1.0 - SQUARE(wnz->data[0])));

        for (k = 1; k < ap->col - 1; k++) {
          if (sigp <= 0.0)
            wnzv = MIN(1.0, pow(10.0, (ap->data[lii][idcv->data[k]]) / 20.0));
          else {
            wnzv = sigmoid(pow(10.0, (ap->data[lii][idcv->data[k]]) / 20.0), sigp, 0.25);
            wnzv = MIN(1.0, wnzv);
          }          
          wnz->data[k] = wnzv;
          wpr->data[k] = sqrt(MAX(0.0, 1.0 - SQUARE(wnzv)));
          wnz->data[fftl - k] = wnzv;
...
/* Load given cepstrum or mel-cepstrum and convert it to spectrum */
                        ...
                        double frame, double fs, double alpha)
{
    ...
    if (float_flag == XTRUE) {
      // memory allocation
      tmpf = xfvalloc(dim);
      while ((size_t)dim == fread(tmpf->data, sizeof(float), (size_t)dim, fp)) 
        len++;

    } else {
      // memory allocation
      tmpd = xdvalloc(dim);
      while ((size_t)dim == fread(tmpd->data, sizeof(double), (size_t)dim, fp)) 
        len++;
      // memory free
      xdvfree(tmpd);
    }
    ...
    for (k = 0; k < len; k++) {
      ...
      if (float_flag == XTRUE) {
        if ((size_t)dim != fread(tmpf->data, sizeof(float), (size_t)dim, fp)) {
          printmsg(stderr, "Error xread_dfcep2spg: %s\n", file);
          return NODATA;
        }
        for (l = 0; l < dim; l++) 
          vec1->data[l] = (double)tmpf->data[l];

      } else {
        if ((size_t)dim != fread(vec1->data, sizeof(double), (size_t)dim, fp)) {
          printmsg(stderr, "Error xread_dfcep2spg: %s\n", file);
          return NODATA;
        }
      }
      ...
      if (chpow_flag == XTRUE)
        tspgpow = pow(10.0, vec1->data[0]) * (frame * fs / 1000.0) / (double)fftl;
      
      if (mel_flag == XTRUE) {
        // transform mel-cepstrum to cepstrum
