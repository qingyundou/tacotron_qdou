...
class DataFeeder_EAL(threading.Thread):
    '''Feeds batches of data into a queue on a background thread.'''

    def __init__(self, coordinator, metadata_filename, hparams, eal_dir):
        super(DataFeeder_EAL, self).__init__()
        self._coord = coordinator
        self._hparams = hparams
        self._cleaner_names = [x.strip() for x in hparams.cleaners.split(',')]
        self._offset = 0
        self._eal_dir = eal_dir
        ...
        self._placeholders = [
            ...
            tf.placeholder(tf.float32, [None, None, hparams.num_freq], 'linear_targets'),
            tf.placeholder(tf.float32, [None, None, hparams.pml_dimension], 'pml_targets'),
            tf.placeholder(tf.float32, [None, None, None], 'locked_alignments'),
        ]

        # Create queue for buffering data:
        queue = tf.FIFOQueue(8, [tf.int32, tf.int32, tf.float32, tf.float32, tf.float32, tf.float32], name='input_queue')
        self._enqueue_op = queue.enqueue(self._placeholders)
        self.inputs, self.input_lengths, self.mel_targets, self.linear_targets, self.pml_targets, self.locked_alignments = queue.dequeue()
        self.inputs.set_shape(self._placeholders[0].shape)
        self.input_lengths.set_shape(self._placeholders[1].shape)
        self.mel_targets.set_shape(self._placeholders[2].shape)
        self.linear_targets.set_shape(self._placeholders[3].shape)
        self.pml_targets.set_shape(self._placeholders[4].shape)
        self.locked_alignments.set_shape(self._placeholders[5].shape)
    ...
    def _enqueue_next_group(self):
        ...
        log('Generated %d batches of size %d in %.03f sec' % (len(batches), n, time.time() - start))
        for batch in batches:
            feed_dict = dict(zip(self._placeholders, _prepare_batch_EAL(batch, r)))
            self._session.run(self._enqueue_op, feed_dict=feed_dict)
...
def _prepare_batch_EAL(batch, outputs_per_step):
    ...
    linear_targets = _prepare_targets([x[2] for x in batch], outputs_per_step)
    pml_targets = _prepare_targets([x[4] for x in batch], outputs_per_step)
    locked_alignments = _prepare_alignments(inputs, [x[5] for x in batch])
#     print(inputs.shape, locked_alignments.shape)
    return inputs, input_lengths, mel_targets, linear_targets, pml_targets, locked_alignments
...
def _prepare_alignments(inputs, locked_alignments):
    max_len = max((len(x) for x in inputs))
    return np.stack([_padORcut_alignment(x, max_len) for x in locked_alignments])
...
def _padORcut_alignment(t, length):
    if length>t.shape[0]:
        return np.pad(t, [(0, length - t.shape[0]), (0, 0)], mode='constant', constant_values=_pad)
    else:
        return t[:length,:]
