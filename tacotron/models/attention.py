...
class LocationSensitiveAttention(BahdanauAttention):
    ...
    def _location_sensitive_score(self, W_query, W_location):
        """
        Implements Bahdanau-style (cumulative) scoring function.
        This attention is described in:
            J. K. Chorowski, D. Bahdanau, D. Serdyuk, K. Cho, and Y. Ben-
        ...
        #############################################################################
                hybrid attention (content-based + location-based)
                                f = F * α_{i-1}
        energy = dot(v_a, tanh(self.keys(h_enc) + W_query(h_dec) + W_location(f) + b_a))
        #############################################################################
        Args:
        ...
        return tf.reduce_sum(v_a * tf.tanh(self.keys + W_query + W_location + b_a), [2])
