...
class TacotronPMLExtendedLocSens_bk2orig:
        ...
        with tf.variable_scope('inference') as scope:
            ...
            if is_training:
                if gta:
                    ...
                    helper = TacoTrainingHelper_EAL(inputs, mel_targets, hp.num_mels, hp.outputs_per_step)
                elif hp.scheduled_sampling:
                    helper = TacoScheduledOutputTrainingHelper(inputs, mel_targets, hp.num_mels, hp.outputs_per_step,
            ...
            if logs_enabled:
                ...
                log('  Decoder out (1 frame):   {}'.format(mel_outputs.shape[-1]))
                log('  Postnet out:             {}'.format(post_outputs.shape[-1]))
                log('  linear out:                 {}'.format(linear_outputs.shape[-1]))
