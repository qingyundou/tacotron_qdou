...
from tacotron.models.helpers import TacoTestHelper, TacoTrainingHelper, TacoTrainingHelper_EAL, TacoScheduledOutputTrainingHelper
...
class TacotronPMLExtendedLocSens_online:
    ...
    def initialize(self, inputs, input_lengths, mel_targets=None, linear_targets=None, pml_targets=None,
                   ...
                   flag_trainAlign=False, flag_trainJoint=False, alignScale=1.0, flag_online_eal_eval=False):
        ...
        self.flag_online_eal = (eal and (locked_alignments is None)) or flag_online_eal_eval
        ...
        with tf.variable_scope('inference') as scope:
            ...
            if gta:
                ...
                if self.flag_online_eal:
                    helper_gta = TacoTrainingHelper(inputs, pml_targets, hp.pml_dimension, hp.outputs_per_step)
                    helper_eal = TacoTrainingHelper_EAL(inputs, pml_targets, hp.pml_dimension, hp.outputs_per_step)
                else:
                ...
                if is_training: log('For training, one of these should be true: gta, eal, hp.scheduled_sampling')
                else:
            ...
            if not self.flag_online_eal:
                ...
                # Grab alignments from the final decoder state:
                locked_alignments_ = tf.transpose(final_decoder_state[0].alignment_history.stack(), [1, 2, 0])
        ...
        with tf.variable_scope('inference_eal') as scope:
            if self.flag_online_eal:
                ...
                embedding_table_eal = tf.get_variable(
                    'embedding', [len(symbols), hp.embed_depth], dtype=tf.float32,
                    initializer=tf.truncated_normal_initializer(stddev=0.5))
                embedded_inputs_eal = tf.nn.embedding_lookup(embedding_table_eal, inputs)  # [N, T_in, embed_depth=256]

                # Encoder
                prenet_outputs_eal = prenet(embedded_inputs_eal, is_training, hp.prenet_depths)  # [N, T_in, prenet_depths[-1]=128]
                encoder_outputs_eal = encoder_cbhg(prenet_outputs_eal, input_lengths, is_training,  # [N, T_in, encoder_depth=256]
                                               hp.encoder_depth)
...
#                 tmp = None if flag_online_eal_eval else locked_alignments_
                ...
                attention_cell_eal = LockableAttentionWrapper(
                    GRUCell(hp.attention_depth),
                    LocationSensitiveAttention(hp.attention_depth, encoder_outputs_eal),
                    alignment_history=True,
                    locked_alignments=locked_alignments_,
                    output_attention=False,
                    name='attention_wrapper',
                ...
                # Apply prenet before concatenation in AttentionWrapper.
                prenet_cell_eal = DecoderPrenetWrapper(attention_cell_eal, is_training, hp.prenet_depths)

                # Concatenate attention context vector and RNN cell output into a 2*attention_depth=512D vector.
                concat_cell_eal = ConcatOutputAndAttentionWrapper(prenet_cell_eal)  # [N, T_in, 2*attention_depth=512]

                # Decoder (layers specified bottom to top):
                decoder_cell_eal = MultiRNNCell([
                    OutputProjectionWrapper(concat_cell_eal, hp.decoder_depth),
                    ResidualWrapper(GRUCell(hp.decoder_depth)),
                    ResidualWrapper(GRUCell(hp.decoder_depth))
                ], state_is_tuple=True)  # [N, T_in, decoder_depth=256]

                # Project onto r PML feature vectors (predict r outputs at each RNN step):
                output_cell_eal = OutputProjectionWrapper(decoder_cell_eal, hp.pml_dimension * hp.outputs_per_step)
                decoder_init_state_eal = output_cell.zero_state(batch_size=batch_size, dtype=tf.float32)

                (decoder_outputs_eal, _), final_decoder_state_eal, _ = tf.contrib.seq2seq.dynamic_decode(
                        BasicDecoder(output_cell_eal, helper_eal, decoder_init_state_eal),
                        maximum_iterations=hp.max_iters)  # [N, T_out/r, P*r]
                
                # Reshape outputs to be one output per entry
                pml_intermediates_eal = tf.reshape(decoder_outputs_eal, [batch_size, -1, hp.pml_dimension])  # [N, T_out, P]

                # Add post-processing CBHG:
                post_outputs_eal = post_cbhg(pml_intermediates_eal, hp.pml_dimension, is_training,  # [N, T_out, postnet_depth=256]
                                         hp.postnet_depth)
                pml_outputs_eal = tf.layers.dense(post_outputs_eal, hp.pml_dimension)  # [N, T_out, P]

                # Grab alignments from the final decoder state:
                alignments = tf.transpose(final_decoder_state_eal[0].alignment_history.stack(), [1, 2, 0])
    ...
    def add_loss(self):
        ...
        with tf.variable_scope('loss') as scope:
            ...
            # avg kl
            if (self.locked_alignments is not None): # or self.flag_online_eal
...
#                                                  self.alignments_clip*tf.log(self.alignments))
            ...
            if self.flag_online_eal:
                self.pml_intermediate_loss_eal = tf.reduce_mean(tf.abs(self.pml_targets - self.pml_intermediates_eal))
                self.pml_loss_eal = tf.reduce_mean(tf.abs(self.pml_targets - self.pml_outputs_eal))
                self.loss_eal = self.pml_intermediate_loss_eal + self.pml_loss_eal
                self.loss_joint_eal = self.loss_eal + self.loss_align*self.alignScale
                self.loss_joint += self.loss_eal
    ...
    def add_optimizer(self, global_step):
        ...
        with tf.variable_scope('optimizer') as scope:
            ...
            if self.flag_trainAlign:
                ...
                if self.flag_online_eal:
                    var_list = [v for v in tf.trainable_variables() if 'inference_eal' not in v.name]
                    var_list_eal = [v for v in tf.trainable_variables() if 'inference_eal' in v.name]
                    gradients, variables = zip(*optimizer.compute_gradients(self.loss, var_list=var_list))
                    gradients_eal, variables_eal = zip(*optimizer.compute_gradients(self.loss_joint_eal, var_list=var_list_eal))
                    gradients += gradients_eal
                    variables += variables_eal
                else:
            ...
            clipped_gradients_eal, _ = tf.clip_by_global_norm(gradients_eal, 1.0)
            ...
            with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
                ...
                self.optimize_eal_only = optimizer.apply_gradients(zip(clipped_gradients_eal, variables_eal),
                                                          global_step=global_step)
                
            self.gradients_fetchable = [g for g in self.gradients if g is not None]
