...
# np.random.seed(1234)
import sys
from tacotron.utils.log_gradients import log_gradients
        ...
        if args.eal_dir:
            if args.eal_dir=='online':
                model.initialize(feeder.inputs, feeder.input_lengths, feeder.mel_targets,
                                 ...
                                 eal=True, locked_alignments=None, 
    ...
    with tf.Session() as sess:
        try:
            ...
            if args.restore_step:
                ...
                if args.eal_dir=='online':
                    list_var = [v for v in tf.trainable_variables() if 'inference_eal' not in v.name]
                    saver_eal = tf.train.Saver(list_var)
                    saver_eal.restore(sess, args.eal_ckpt)
                    log('Loaded weights of checkpoint: %s at commit: %s' % (args.eal_ckpt, commit), slack=True)
                elif args.eal_trainAlign or args.eal_trainJoint:
...
#                 if step%3==0: import pdb; pdb.set_trace()
                ...
                if args.eal_trainAlign:
                    ...
                    if args.eal_dir=='online':
                        try:
                            if step < args.eal_only_start:
                                step, loss, loss_eal, loss_align, loss_joint, opt = sess.run([global_step, model.loss, model.loss_eal,
                                                                                              model.loss_align, model.loss_joint,
                                                                                              model.optimize])
                            else:
                                step, loss, loss_eal, loss_align, loss_joint, opt = sess.run([global_step, model.loss, model.loss_eal,
                                                                                              model.loss_align, model.loss_joint,
                                                                                              model.optimize_eal_only])
                        except Exception as e:
                            log('Exiting due to exception: %s' % e, slack=True)
                            log_gradients(sess, model, log)
                            traceback.print_exc()
                            coord.request_stop(e)
                            
                        time_window.append(time.time() - start_time)
                        loss_window.append(loss_joint)
                        message = 'Step %-7d [%.03f sec/step, loss=%.05f, loss_eal=%.05f, loss_align=%.05f, avg_loss_joint=%.05f]' % (
                            step, time_window.average, loss, loss_eal, loss_align, loss_window.average)
                    else:
                ...
                if step % args.checkpoint_interval == 0:
                    ...
                    # otherwise, synthesize audio from PML vocoder features
                        ...
                        if args.eal_dir=='online':
                            input_seq, locked_alignment, alignment, target_pml_features, pml_features, pml_features_eal = sess.run([
                                model.inputs[0], model.locked_alignments[0], model.alignments[0], 
                                model.pml_targets[0], model.pml_outputs[0], model.pml_outputs_eal[0]])
                        else:
                        ...
                        if args.eal_dir=='online':
                            output_waveform_eal = synth.pml_to_wav(pml_features_eal, mean_norm=mean_norm, std_norm=std_norm,
                                                                   spec_type=hparams.spec_type)
                            sp.wavwrite(os.path.join(log_dir, 'step-%d-audio-eal.wav' % step), output_waveform_eal,
                                        hparams.sample_rate, norm_max_ifneeded=True)
                    ...
                    if args.eal_dir=='online':
                        random_locked_attention_plot = plot.plot_alignment(locked_alignment, os.path.join(log_dir,
                                                                                            'step-%d-random-locked-align.png' % step),
                                                                    info='%s, %s, %s, step=%d, loss=%.5f' % (
                                                                    args.variant, commit, time_string(), step, loss))
                        
                        summary_elements.append(
                            tf.summary.image('locked-attention-%d' % step, random_locked_attention_plot),
                        )
