    ...
    try:
      ...
      while not coord.should_stop() and step <= args.num_steps:
        ...
        if step % args.checkpoint_interval == 0:
          ...
          input_seq, target_pml_features, pml_features, alignment = sess.run([
            model.inputs[0], model.pml_targets[0], model.pml_outputs[0], model.alignments[0]])

          # generate an attention plot image
          attention_plot = plot.plot_alignment(alignment, os.path.join(log_dir, 'step-%d-align.png' % step),
            info='%s, %s, %s, step=%d, loss=%.5f' % (args.model, commit, time_string(), step, loss))

          # save the alignment to tensorboard
